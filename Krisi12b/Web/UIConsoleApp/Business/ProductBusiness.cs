...
namespace Business
{
    public class ProductBusiness
    {
        ...
        public string GetAll()
        {
            ...
            {
                var products = productContext.Products.Select(p => new
                {
                    p.Name,
                    p.Price,
        ...
        public void AddProduct(Product product, Category category)
        {
            using (productContext = new ProductContext())
            {
                var cattegorySearch = productContext.Categories.FirstOrDefault(c => c.Name == category.Name);
                if (cattegorySearch == null)
                {
                ...
                {
                    product.CategoryId = cattegorySearch.Id;
                    productContext.Products.Add(product);
                    productContext.SaveChanges();
                }
            }
        ...
        public void UpdateProduct(Product product, Category category)
        {
            using (productContext = new ProductContext())
            {
                ...
                {
                    product.CategoryId = categorySearch.Id;
                }
